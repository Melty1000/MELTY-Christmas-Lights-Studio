...
import { initPostFX, sceneBloom, createBloomClone, syncBloomTransforms, renderWithPostFX, resizePostFX } from './postfx.js';
...
export function initScene() {
    ...
    console.log('🎨 Creating bloom clones for', activeLights.length, 'light groups...');
    console.log('📊 activeLights array:', activeLights);
    sceneBloom.clear(); // Clear any previous clones

    let cloneCount = 0;
    activeLights.forEach((bulb, idx) => {
        console.log(`🔍 Light ${idx}:`, bulb.type, bulb.children.length, 'children');

        // Find the glass mesh (which has emissive material)
        bulb.traverse(child => {
            if (child.isMesh) {
                console.log(`  → Mesh found:`, child.geometry.type, 'Material:', child.material?.type, 'Has emissive?', !!child.material?.emissive);

                if (child.material && child.material.emissive) {
                    console.log(`  ✅ Creating bloom clone for emissive mesh`);
                    const clone = createBloomClone(child);
                    if (clone) {
                        sceneBloom.add(clone);
                        cloneCount++;
                    }
                }
            }
    ...
    console.log(`✅ Created ${cloneCount} bloom clones`);
    console.log('📦 sceneBloom children:', sceneBloom.children.length);
